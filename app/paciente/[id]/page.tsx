            ...
            {esFamilia && editandoApodo ? (
              <div className="flex items-center gap-2 mt-1.5">
                ...
                <button
                  ...
                  disabled={guardandoApodo}
                  className="text-[#1A6BFF] text-xs font-semibold disabled:opacity-50"
                >
                  {guardandoApodo ? "Guardando..." : "Guardar"}
                </button>
                <button
                  onClick={() => { setApodoInput(paciente.apodo || ""); setEditandoApodo(false); }}
              ...
              <p className="text-slate-500 text-sm mt-1 flex items-center gap-2 flex-wrap">
                ...
                {esFamilia && (
                  <button
                    ...
                    className="text-slate-400 hover:text-[#1A6BFF] text-xs transition-colors"
                  >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {esFamilia && (
            <Link href={`/paciente/${params.id}/equipo`} className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-[#1A6BFF] hover:shadow-md transition-all shadow-sm cursor-pointer block no-underline">
              <p className="text-2xl mb-3">🩺</p>
              <h2 className="text-slate-900 font-semibold mb-1">Equipo de {nombreParaEquipo}</h2>
          ...
          {(esFamilia || esProfesionalClinico) && (
            <Link href={`/paciente/${params.id}/notas`} className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-[#1A6BFF] hover:shadow-md transition-all shadow-sm cursor-pointer block no-underline">
              ...
              <p className="text-slate-500 text-sm">
                {esProfesionalClinico ? "Consultas, sesiones y reportes — puedes agregar y firmar" : "Consultas, sesiones y reportes del equipo"}
              </p>
            </Link>
          )}

          {(esFamilia || esProfesionalClinico) && (
            <Link href={`/paciente/${params.id}/bitacora-familiar`} className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-[#1A6BFF] hover:shadow-md transition-all shadow-sm cursor-pointer block no-underline">
              <p className="text-2xl mb-3">📝</p>
              <h2 className="text-slate-900 font-semibold mb-1">Bitácora familiar</h2>
              <p className="text-slate-500 text-sm">
                {esFamilia ? "Tu registro libre del día a día" : "Registro libre de la familia (contexto, no clínico)"}
              </p>
            </Link>
          )}
